...
class UNBCMcMasterDatasetVideo(Dataset):
    def __init__(self, video_dir, label_dir, val_subj_id, test_subj_id, subset, transform=None, labeltransform=None, frame_score_dir = None, iau_dir = None):
        """
        Args:
            video_dir (string): Path to the video data "detected"
        ...
        self.video_files = []
        for dir_s in next(os.walk(video_dir))[1]:
            for dir_v in next(os.walk(os.path.join(video_dir, dir_s)))[1]:
                if ((dir_s[:3] in test_subj_id and subset=='test') or (dir_s[:3] in val_subj_id and subset=='val') or (not(dir_s[:3] in val_subj_id + test_subj_id) and subset=='train')):
                    self.video_files.append((dir_s, dir_v))
